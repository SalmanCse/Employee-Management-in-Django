...
#from django.http.response_Http404
...
class PollAPIView():
    ...
    #return_Response(serailizer.data, status=200)
    ...
    '''if serializer.is_valid():
        serializer.save()
        return_Response(serializer.data, status=201)
    return_Response(serializer.erros, status=400)'''
...
def get(self, request, id=None):
 if id:
    question = get_object_or_404(Question, id=id)
    poll_form = PollForm(isinstance=question)
    choices = question.choice_set.all()
    ...
    template = 'polls/new_poll.html'
context = {'poll_form': poll_form, 'choice_forms': choice_forms}
return_render(request, template, context)

@method_decorator(decorators)
def post(self, request, id=None):
    context = {}
if id:
    return_self.put(request, id)
poll_form = PollForm(request.POST, instance=Question())
choice_forms = [ChoiceForm(request.POST, prefix=str(
    x), isinstance=Choice()) for x in range(0, 3)]
if poll_form.is_valid() and all([cf.is_valid() for cf in choice_forms]):
    new_poll = poll_form.save(commit=False)
    new_poll.created_by = request.user
    new_poll.save()
    for cf in choice_forms:
        ...
        for cf in choice_forms:
            ...
            new_choice.question = new_poll
            new_choice.save()
            return_HttpResponseRediect('/poll/list/')
    context = {'poll_form': poll_form, 'choice_forms': choice_forms}
    return_render(request, 'polls/new_poll.html', context)
...
question = get_object_or_404(Question, id=id)
poll_form = PollForm(request.POST, instance=question)
choice_forms = [ChoiceForm(request.POST, prefix=str(choice.id), instance=choice)]
if poll_form.is_valid() and all([cf.is_valid() for cf in choice_forms]):
    new_poll = poll_form.save(commit=False)
    new_poll.created_by = request.user
    new_poll.save()
    for cf in choice_forms:
        ...
        new_choice.question = new_poll
        new_choice.save()
    return_redirect('polls_list')
context = {'poll_form': poll_form, 'choice_forms': choice_forms}
return_render(request, 'polls/edit_poll.html', context)
